...
void VideoDriver_SDL3_Base::MakeDirty(int left, int top, int width, int height)
{
	Rect r = {left, top, left + width, top + height};
	this->dirty_rect = BoundingRect(this->dirty_rect, r);
}

void VideoDriver_SDL3_Base::CheckPaletteAnim()
{
...
void VideoDriver_SDL3_Base::ClientSizeChanged(int w, int h, bool force)
{
...
bool VideoDriver_SDL3_Base::CreateMainWindow(uint w, uint h, uint flags)
{
    ...
    SDL_SetStringProperty(props, SDL_PROP_WINDOW_CREATE_TITLE_STRING, caption.c_str());
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_X_NUMBER, x);
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_Y_NUMBER, y);
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_WIDTH_NUMBER, w);
    SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_HEIGHT_NUMBER, h);
	SDL_SetNumberProperty(props, SDL_PROP_WINDOW_CREATE_FLAGS_NUMBER, flags);
	this->sdl_window = SDL_CreateWindowWithProperties(props);
	...
	if (!icon_path.empty()) {
		...
		if (icon != nullptr) {
			...
			const SDL_PixelFormatDetails *fmt = SDL_GetPixelFormatDetails(icon->format);
			uint32_t rgbmap = SDL_MapRGB(fmt, NULL, 255, 0, 255);

			SDL_SetSurfaceColorKey(icon, true, rgbmap);
...
bool VideoDriver_SDL3_Base::CreateMainSurface(uint w, uint h, bool resize)
{
...
bool VideoDriver_SDL3_Base::ClaimMousePointer()
{
 ...
 */
void VideoDriver_SDL3_Base::EditBoxGainedFocus()
{
 ...
 */
void VideoDriver_SDL3_Base::EditBoxLostFocus()
{
	if (this->edit_box_focused) {
...
std::vector<int> VideoDriver_SDL3_Base::GetListOfMonitorRefreshRates()
{
	SDL_DisplayID *displays;
	SDL_DisplayMode **modes;
...
struct SDL3VkMapping {
	const SDL_Keycode vk_from;
	const uint8_t vk_count;
	const uint8_t map_to;
	const bool unprintable;

	constexpr SDL3VkMapping(SDL_Keycode vk_first, SDL_Keycode vk_last, uint8_t map_first, [[maybe_unused]] uint8_t map_last, bool unprintable)
...
static constexpr SDL3VkMapping _vk_mapping[] = {
...
static uint ConvertSdlKeyIntoMy(SDL_KeyboardEvent *kb, char32_t *character)
{
	uint key = 0;
	bool unprintable = false;

	for (const auto &map : _vk_mapping) {
		if (IsInsideBS(kb->key, map.vk_from, map.vk_count)) {
			key = kb->key - map.vk_from + map.map_to;
	...
	if (kb->scancode == SDL_SCANCODE_GRAVE) key = WKC_BACKQUOTE;

	/* META are the command keys on mac */
	if (kb->mod & SDL_KMOD_GUI)   key |= WKC_META;
	if (kb->mod & SDL_KMOD_SHIFT) key |= WKC_SHIFT;
	if (kb->mod & SDL_KMOD_CTRL)  key |= WKC_CTRL;
	if (kb->mod & SDL_KMOD_ALT)   key |= WKC_ALT;

	/* The mod keys have no character. Prevent '?' */
	if (kb->mod & SDL_KMOD_GUI ||
		kb->mod & SDL_KMOD_CTRL ||
		kb->mod & SDL_KMOD_ALT ||
		unprintable) {
		*character = WKC_NONE;
	} else {
...
static uint ConvertSdlKeycodeIntoMy(SDL_Keycode kc)
{
	...
	SDL_Scancode sc = SDL_GetScancodeFromKey(kc, NULL);
	if (sc == SDL_SCANCODE_GRAVE) key = WKC_BACKQUOTE;

	return key;
}

bool VideoDriver_SDL3_Base::PollEvent()
{
	SDL_Event ev;
	...
	switch (ev.type) {
		...
		case SDL_EVENT_MOUSE_WHEEL: {
			...
			_cursor.v_wheel -= ev.wheel.x * SCROLL_BUILTIN_MULTIPLIER * _settings_client.gui.scrollwheel_multiplier;
			_cursor.h_wheel += ev.wheel.y * SCROLL_BUILTIN_MULTIPLIER * _settings_client.gui.scrollwheel_multiplier;
		...
		case SDL_EVENT_WINDOW_RESIZED: case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED: {
				Debug(driver, 1, "SDL3: window resized to {}x{}", ev.window.data1, ev.window.data2);
				// TODO(mauri870): SDL3 bug? data1 and data2 are always zero.
				int w = std::max(ev.window.data1, 640);
				int h = std::max(ev.window.data2, 480);
...
static std::optional<std::string_view> InitializeSDL3()
{
	Debug(driver, 1, "SDL3: Version {}.{}.{}", SDL_MAJOR_VERSION, SDL_MINOR_VERSION, SDL_MICRO_VERSION);
...
std::optional<std::string_view> VideoDriver_SDL3_Base::Initialize()
{
	this->UpdateAutoResolution();

	auto error = InitializeSDL3();
...
std::optional<std::string_view> VideoDriver_SDL3_Base::Start(const StringList &param)
{
...
void VideoDriver_SDL3_Base::Stop()
{
...
void VideoDriver_SDL3_Base::InputLoop()
{
	...
	this->fast_forward_key_pressed = keys[SDL_SCANCODE_TAB] && (mod & SDL_KMOD_ALT) == 0;
...
void VideoDriver_SDL3_Base::LoopOnce()
{
	if (_exit_game) {
#ifdef __EMSCRIPTEN__
...
void VideoDriver_SDL3_Base::MainLoop()
{
#ifdef __EMSCRIPTEN__
...
bool VideoDriver_SDL3_Base::ChangeResolution(int w, int h)
{
	return CreateMainSurface(w, h, true);
}

bool VideoDriver_SDL3_Base::ToggleFullscreen(bool fullscreen)
{
	...
	if (fullscreen) {
		...
		mode = SDL_GetCurrentDisplayMode(SDL_GetDisplayForWindow(this->sdl_window));
		if (mode == nullptr) {
			Debug(driver, 0, "SDL_GetCurrentDisplayMode() failed: {}", SDL_GetError());
		} else {
			SDL_SetWindowSize(this->sdl_window, mode->w, mode->h);
		}
	}

	Debug(driver, 1, "SDL3: Setting {}", fullscreen ? "fullscreen" : "windowed");
	bool ret = SDL_SetWindowFullscreen(this->sdl_window, fullscreen ? SDL_WINDOW_FULLSCREEN : 0);
	if (!ret) {
...
bool VideoDriver_SDL3_Base::AfterBlitterChange()
{
...
Dimension VideoDriver_SDL3_Base::GetScreenSize() const
{
	const SDL_DisplayMode *mode;
	mode = SDL_GetCurrentDisplayMode(this->startup_display);
	if (mode == nullptr) {
		return VideoDriver::GetScreenSize();
	}

	Dimension dimension = { static_cast<uint>(mode->w), static_cast<uint>(mode->h) };

	return dimension;
}

bool VideoDriver_SDL3_Base::LockVideoBuffer()
{
...
void VideoDriver_SDL3_Base::UnlockVideoBuffer()
{
